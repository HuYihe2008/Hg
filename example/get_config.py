...
# remote_config（network_config/game_config）：key 固定；iv 不固定 = base64解码后的前16字节
ENDFIELD_RC_KEY_B64_OVERSEA = "cZm86UfDp/kgJ3agKx+HZA=="
ENDFIELD_RC_KEY_B64_CN = "Wgxugl5qVirx7r3km6nXtA=="

# u8ExtraConfig.bin：key/iv 固定
...
class EndfieldConfigFetcher:
    ...
    def get_engine_config(self, device: str) -> dict[str, Any]:
        return self._get_json(ENGINE_CONFIG_URL.format(device="default")) #只有default能用

    def get_network_config(self, device: str) -> dict[str, Any]:
        return self._get_remote_config(NETWORK_CONFIG_URL.format(device="default")) #只有default能用
...
def main():
    ...
    fetcher = EndfieldConfigFetcher(is_oversea=oversea) #不想做
